...
public class VariableNameGenerator {
    ...
    public String makeHeaderDefineFromName(MenuItem item) {
        var variable = makeNameToVar(item, null);
        if (variable.isEmpty()) return "";

        StringBuilder result = new StringBuilder();
        for (int i = 0; i < variable.length(); i++) {
            char ch = variable.charAt(i);
            if (Character.isUpperCase(ch) && i > 0 && !Character.isUpperCase(variable.charAt(i - 1)) && Character.isLetterOrDigit(variable.charAt(i - 1))) {
                result.append('_');
            }
