...
public class JsonCodeGeneratorOptionsSerialisation {
    ...
    public static final String JSON_USE_DYNAMIC_MENUS = "useDynamicMenus";
    public static final String JSON_SIZE_EEPROM = "usingSizedEEPROMStorage";
    public static final String JSON_EEPROM_MODE = "eepromSaveMode";
    ...
    class CodeGeneratorOptionsDeseriailiser implements JsonDeserializer<CodeGeneratorOptions> {
        ...
        public CodeGeneratorOptions deserialize(JsonElement jsonElement, Type type, JsonDeserializationContext ctx) throws JsonParseException {
            ...
            builder.withPlatform(platforms.getEmbeddedPlatformFromId(platformText))
                    ...
                    .withEepromSaveMode(findSaveMode(jsonObj))
                    .withDynamicMenus(optionalElement(jsonObj, JSON_USE_DYNAMIC_MENUS, false));
        ...
        private EepromSaveMode findSaveMode(JsonObject jsonObj) {
            if(jsonObj.has(JSON_EEPROM_MODE)) {
                return EepromSaveMode.valueOf(jsonObj.get(JSON_EEPROM_MODE).getAsString());
            } else if(jsonObj.has(JSON_SIZE_EEPROM)) {
                return jsonObj.get(JSON_SIZE_EEPROM).getAsBoolean() ? EepromSaveMode.WRITE_BY_POSITION_WITH_SIZE : EepromSaveMode.LEGACY_WRITE_BY_POSITION;
            } else {
                return EepromSaveMode.DYNAMIC_WRITE_BY_ID;
            }
        }
    ...
    class CodeGeneratorOptionsSeriailiser implements JsonSerializer<CodeGeneratorOptions> {
        ...
        public JsonElement serialize(CodeGeneratorOptions opts, Type type, JsonSerializationContext ctx) {
            ...
            obj.addProperty(JSON_USE_DYNAMIC_MENUS, opts.isUseDynamicMenus());
            obj.addProperty(JSON_SIZE_EEPROM, opts.isUsingSizedEEPROMStorage());
            obj.addProperty(JSON_EEPROM_MODE, opts.getEepromSaveMode().toString());
        ...
        private JsonElement serialiseListOfForms(List<String> listOfEmbeddedForms) {
            var arr = new JsonArray();
            for(var form : listOfEmbeddedForms) {
                arr.add(form);
            }
            return arr;
        }
