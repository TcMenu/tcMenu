...
public class FileBasedProjectPersistor implements ProjectPersistor {
    ...
    public MenuTreeWithCodeOptions open(String fileName) throws IOException {
        ...
        try (Reader reader = new BufferedReader(new FileReader(fileName))) {
            return getMenuTreeWithCodeOptions(reader);
        }
    }

    public MenuTreeWithCodeOptions getMenuTreeWithCodeOptions(Reader reader) {
        PersistedProject prj = serializer.getGson().fromJson(reader, PersistedProject.class);
        return getMenuTreeWithCodeOptions(prj);
    }

    public MenuTreeWithCodeOptions getMenuTreeWithCodeOptions(PersistedProject prj) {
    ...
    public PersistedProject asPersistedProject(MenuTreeWithCodeOptions menuTreeWithCodeOptions) {
        var items = new ArrayList<PersistedMenu>();
        for(var item : menuTreeWithCodeOptions.getMenuTree().getAllMenuItems()) {
            items.add(new PersistedMenu(menuTreeWithCodeOptions.getMenuTree().findParent(item), item));
        }
        return new PersistedProject(menuTreeWithCodeOptions.getDescription(), "WebUser", items,
                menuTreeWithCodeOptions.getOptions(), new PersistableStringList[0]);
    }
