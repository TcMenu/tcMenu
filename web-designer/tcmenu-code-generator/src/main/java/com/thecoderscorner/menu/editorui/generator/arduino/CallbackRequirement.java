...
import static com.thecoderscorner.menu.editorui.UrlsForDocumentation.*;
...
public class CallbackRequirement {
    ...
    private static final Map<String, String> ADDED_CALLBACK_MAP = Map.of(
            "tcAdded_onAddToCount", "getMenuCount().setCurrentValue(getMenuCount().getCurrentValue() + 1); // Added by generator for the count example"
    );
    ...
    public List<String> generateSketchCallback() {
        return MenuItemHelper.visitWithResult(callbackItem, new AbstractMenuItemVisitor<List<String>>() {
            ...
            public void visit(ScrollChoiceMenuItem scrollItem) {
                if(scrollItem.getChoiceMode() == ScrollChoiceMode.CUSTOM_RENDERFN) {
            ...
            private void runtimeCustomCallback(MenuItem item) {
                ...
                if(!(item instanceof RuntimeListMenuItem rli) || rli.getListCreationMode() == ListCreationMode.CUSTOM_RTCALL) {
                    customCb = List.of(
            ...
            private List<String> standardCallbackFunction() {
                String bodySnippet = ADDED_CALLBACK_MAP.getOrDefault(callbackName, "// TODO - your menu change code");
                return List.of(
                        "",
    ...
    public List<String> generateSource() {
        return MenuItemHelper.visitWithResult(callbackItem, new AbstractMenuItemVisitor<List<String>>() {
            ...
            private void generateSourceForEditableRuntime(MenuItem item) {
                ...
                } else if (item instanceof ScrollChoiceMenuItem sc && sc.getChoiceMode() == ScrollChoiceMode.ARRAY_IN_RAM) {
            ...
            public void visit(ScrollChoiceMenuItem item) {
                if(item.getChoiceMode() != ScrollChoiceMode.CUSTOM_RENDERFN) {
                    generateSourceForEditableRuntime(item);
                }
                else anyItem(item);
            }
    ...
    public String generateHeader() {
        return MenuItemHelper.visitWithResult(callbackItem, new AbstractMenuItemVisitor<String>() {
            ...
            public void visit(RuntimeListMenuItem listItem) {
                String extra = "";
                if(listItem.getListCreationMode() == ListCreationMode.CUSTOM_RTCALL) {
            ...
            public void visit(ScrollChoiceMenuItem choiceMenuItem) {
                if(choiceMenuItem.getChoiceMode() == ScrollChoiceMode.CUSTOM_RENDERFN) {
    ...
    public String getCallbackName() {
        return MenuItemHelper.visitWithResult(callbackItem, new AbstractMenuItemVisitor<String>() {
            ...
            public void visit(ScrollChoiceMenuItem scrollChoiceItem) {
                if(scrollChoiceItem.getChoiceMode() == ScrollChoiceMode.CUSTOM_RENDERFN) {
    ...
    public boolean isCallbackGenerationNeeded(Set<String> definedList) {
        if(callbackItem instanceof RuntimeListMenuItem rtl) {
            if(rtl.getListCreationMode() == ListCreationMode.CUSTOM_RTCALL) {
                return !definedList.contains(generator.makeRtFunctionName(callbackItem)); // we always create list custom functions, they are always unique
            }
        } else if(callbackItem instanceof ScrollChoiceMenuItem scl) {
            if(scl.getChoiceMode() == ScrollChoiceMode.CUSTOM_RENDERFN) {
                return !definedList.contains(generator.makeRtFunctionName(callbackItem)); // we always create list custom functions, they are always unique
            }
        }
        return !definedList.contains(getCallbackName()) && StringUtils.hasLength(getCallbackName());
    }
}
