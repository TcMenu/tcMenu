...
public class WrapWebServerFilesCommand implements Callable<Integer> {
    ...
    public Integer call() throws Exception {
        ...
        for (var file : allFiles) {
            if (file.toFile().isDirectory() || (!wantMaps && file.toString().endsWith(".map"))) {
            ...
            if (shouldGzip && !binFile) {
                ...
                System.out.println("Small binary " + file + " size is " + size);
                var lines = Files.readAllLines(file);
                lines.forEach(l -> sbVariables.append("    \"").append(l.replace("\"", "\\\""))
                        .append("\" \\").append(LINE_BREAK));
                entireSize += size;
            } else {
                System.out.println("Text " + file + " size is " + size);
                var allBytes = Files.readAllBytes(file);
                processAsBinary(sbVariables, allBytes);
                entireSize += size;
            }
            ...
            sbFunctions.append(writeServeLogicFor(theLocalPath, varName, binFile));
        }
    ...
    private String writeServeLogicFor(String theLocalPath, String varName, boolean binary) {
        String mimeType = mimeTypeFor(theLocalPath);
        return switch (creationMode) {
            case ESP_ASYNC -> writeServeLogicEspAsync(theLocalPath, varName, mimeType, binary);
            case AVR_WEBSERVER -> "TODO";
        };
    }

    private String writeServeLogicEspAsync(String theLocalPath, String varName, String mime, boolean binary) {
